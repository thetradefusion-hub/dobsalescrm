...
export function PipelineAnalytics({ stages, deals }: PipelineAnalyticsProps) {
  ...
  return (
    <TooltipProvider>
      <div className="grid grid-cols-2 gap-3 rounded-xl border border-wa-border bg-wa-panel/60 p-4 sm:grid-cols-3 xl:grid-cols-6">
        <Metric
          icon={<BarChart3 className="h-4 w-4 text-wa-muted" />}
          label="Total Deals"
          value={String(stats.totalCount)}
          tooltip="Count of every deal in this pipeline that isn't marked as Lost. Won deals are still included."
        />
        <Metric
          icon={<DollarSign className="h-4 w-4 text-wa-green" />}
          label="Pipeline Value"
          value={formatDealCurrency(stats.totalValue)}
        ...
        />
        <Metric
          icon={<Trophy className="h-4 w-4 text-wa-green" />}
...
function Metric({
  ...
  return (
    <div className="rounded-lg bg-wa-surface/50 p-3">
          ...
          <TooltipTrigger
            render={
              <button
                ...
                className="ml-auto text-wa-muted/80 hover:text-wa-text/90 focus:outline-none"
              />
            }
          >
      ...
      <p className="mt-1 text-base font-semibold text-wa-text">{value}</p>
    </div>
  );
}
