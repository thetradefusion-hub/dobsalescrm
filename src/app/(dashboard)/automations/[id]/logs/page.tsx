    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <button
          ...
          className="flex h-8 w-8 items-center justify-center rounded-md text-wa-muted transition-colors hover:bg-wa-surface hover:text-wa-text"
          aria-label="Back"
        >
          <ArrowLeft className="h-4 w-4" />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-wa-text">{automation.name}</h1>
          <p className="mt-0.5 text-sm text-wa-muted">Execution logs</p>
        </div>
      </div>
      ...
      {logs.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-wa-border bg-wa-panel/40">
          <p className="text-sm text-wa-text">No executions yet</p>
        ...
        <ul className="space-y-2">
          {logs.map((log) => {
            ...
            return (
              <li
                key={log.id}
                className="rounded-xl border border-wa-border bg-wa-panel"
              >
                <button
                  type="button"
                ...
                >
                  {isOpen ? (
                    <ChevronDown className="h-4 w-4 text-wa-muted/80" />
                  ) : (
                    <ChevronRight className="h-4 w-4 text-wa-muted/80" />
                  )}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="truncate text-xs text-wa-muted/80">
                      {log.trigger_event} · {log.steps_executed?.length ?? 0} step
                      {log.steps_executed?.length === 1 ? "" : "s"}
                    </div>
                  </div>
                  <div className="text-xs text-wa-muted/80">
                  ...
                  <div className="border-t border-wa-border px-4 py-3">
                    ...
                    <ul className="space-y-1.5">
                      ...
                      {(log.steps_executed ?? []).length === 0 && (
                        <li className="text-xs text-wa-muted/80">No steps recorded.</li>
                      )}
                    </ul>
                  </div>
                )}
...
function StatusBadge({ status }: { status: AutomationLog["status"] }) {
  const classes =
    status === "success"
      ? "border-wa-green/30 bg-wa-green/10 text-wa-green/90"
      : status === "partial"
    ...
    <li className="flex items-start gap-2 text-xs">
      <span
        className={cn(
          ...
          ok ? "bg-wa-green/20 text-wa-green" : "bg-red-500/20 text-red-400",
        )}
        aria-hidden
      >
        {ok ? <Check className="h-3 w-3" /> : <X className="h-3 w-3" />}
      </span>
      <span className="text-wa-text/90">{result.step_type}</span>
      {result.detail && (
        <span className="truncate text-wa-muted/80">— {result.detail}</span>
      )}
    </li>
  )
}
