  ...
  return (
    <div className="rounded-xl border border-wa-border bg-wa-panel p-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-wa-muted/80">{pct}%</span>
      </div>
      <p className="mt-3 text-2xl font-bold text-wa-text">{value.toLocaleString()}</p>
...
function FunnelChart({ steps }: { steps: FunnelStep[] }) {
  ...
  return (
    <div className="rounded-xl border border-wa-border bg-wa-panel p-4">
      <h3 className="mb-4 text-sm font-medium text-wa-text">Funnel</h3>
        ...
        {steps.map((step) => {
          ...
          return (
            <div key={step.label} className="flex items-center gap-3">
              <span className="w-20 shrink-0 text-xs text-wa-muted">
                {step.label}
              </span>
              <div className="relative h-7 flex-1 rounded-full bg-wa-surface">
                <div
                  className={`h-7 rounded-full ${step.color} transition-[width] duration-500`}
                  style={{ width: `${pctOfMax}%` }}
                />
                <span className="absolute inset-0 flex items-center px-3 text-xs font-medium text-wa-text">
                  {step.value.toLocaleString()}
                  <span className="ml-2 text-wa-text/90/80">
                    ({pctOfSent}%)
                  </span>
                </span>
              </div>
...
export default function BroadcastDetailPage() {
  ...
  const funnelSteps: FunnelStep[] = [
    { label: 'Sent', value: broadcast.sent_count, color: 'bg-wa-green' },
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-wa-text">{broadcast.name}</h1>
              <span
            ...
            <div className="mt-1 flex items-center gap-3 text-sm text-wa-muted">
        ...
        {confirmDelete ? (
          <div className="flex items-center gap-2 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-1.5 text-sm">
            ...
            <Button
              ...
              disabled={deleting}
              className="h-7 border-wa-border bg-transparent text-wa-text/90 hover:bg-wa-surface"
            >
              Cancel
            </Button>
            <Button
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <StatCard
          ...
          color="bg-wa-surface text-wa-text/90"
        />
        <StatCard
          label="Sent"
      ...
      <div className="rounded-xl border border-wa-border bg-wa-panel">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-wa-border px-4 py-3">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="border-wa-border bg-wa-panel">
                <DropdownMenuItem
                  ...
                  className={
                    statusFilter === 'all' ? 'text-wa-green' : 'text-wa-text/90'
                  }
                >
                  All statuses
                </DropdownMenuItem>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-wa-border hover:bg-transparent">
                  ...
                  <TableHead className="text-wa-muted">Sent</TableHead>
                  <TableHead className="text-wa-muted">Delivered</TableHead>
                  <TableHead className="text-wa-muted">Read</TableHead>
                  <TableHead className="text-wa-muted">Error</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredRecipients.map((recipient) => {
                  ...
                  return (
                    <TableRow key={recipient.id} className="border-wa-border">
                      ...
                      <TableCell className="text-wa-muted">
                        {recipient.sent_at
